 ...
 */
abstract public class CacheDataRepository<Service, ResultModel> extends
    ...
    public CacheDataRepository(ApiService.Builder apiBuilder) {
        super(apiBuilder);
        mData = initialData();
    }
     ...
     * @return
     */
    public Observable<ResultModel> updateAfter(CacheDataRepository<Service, ResultModel> first) {
        return updateAfter(first.getUpdatableData());
    }
    ...
    public <T> Observable<ResultModel> updateAfter(Observable<T> first) {
        return Observable
                .zip(first, getUpdatableData(), (firstT, resultModel) -> resultModel)
                .observeOn(THREAD_MAIN)
                .subscribeOn(THREAD_IO);
    }
    ...
    abstract protected Observable<ResultModel> getUpdatableData();

    protected CacheDataRepository<Service, ResultModel> updateDelayedInternal(
    ...
    protected CacheDataRepository<Service, ResultModel> updateInternal(
            boolean force,
